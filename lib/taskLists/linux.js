...
exports.deploy = function(bundlePath, env, config) {
  ...
  taskList.copy('Copy docker', {
    src: path.resolve(TEMPLATES_DIR, 'Dockerfile'),
    dest: '/opt/' + appName + '/config/Dockerfile',
    vars: {
      appName: appName,
    }
  });
  ...
  taskList.copy('Initializing start script', {
    src: path.resolve(TEMPLATES_DIR, 'start-with-swarm.sh'),
    dest: '/opt/' + appName + '/config/start-with-swarm.sh',
    vars: {
      appName: appName,
...
function copyEnvVars(taskList, env, appName) {
  ...
  // Changing the port is done via the start-with-swarm.sh script
  delete env.PORT;
...
function startAndVerify(taskList, appName, port, deployCheckWaitTime) {
  taskList.execute('Starting app', {
    command: "bash /opt/" + appName + "/config/start-with-swarm.sh"
  });
